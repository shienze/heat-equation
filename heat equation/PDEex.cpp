...
void PDEex::Solve(const int& k, int& T)
{
    //k=1, ‰≥ˆƒ≥“ª ±øÃœ¬∏˜µ„Œª÷√µƒΩ·π˚  int(T / Dt)
    if (k == 1)
    {
        if (T > Nt/Dt) T = Nt/Dt;
        cout << "Solve T = " << T << endl;
        for (int i = 0; i < T; ++i)
        {
            for (int j = 1; j < N; ++j)
            {
                Un[j] = Dtx * U0[j - 1] + (1.0 - 2.0 * Dtx) * U0[j] + Dtx * U0[j + 1];
            }
            Un[0] = Un[1];
            Un[N] = Un[N - 1];
            U0 = Un;
        }
        fileout = "output_U_T1.txt";
